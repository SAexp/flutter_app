...
class ILiveHereSupabaseUser extends BaseAuthUser {
  ILiveHereSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> iLiveHereSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ILiveHereSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
